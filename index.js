...
module.exports = async (req, res) => {
    ...
    if (isRootUrl(req.url)) {
        if (req.method === 'POST') {
            ...
            console.log(`No matching method for root URL: Method = ${req.method}`);
            return res.status(405).json({ message: 'Method Not Allowed for root URL', endpoint: req.url, method: req.method });
        }
    } else {
        console.log(`No matching route for URL = ${req.url}`);
        return res.status(404).json({ message: 'Not Found', endpoint: req.url, method: req.method });
    }
